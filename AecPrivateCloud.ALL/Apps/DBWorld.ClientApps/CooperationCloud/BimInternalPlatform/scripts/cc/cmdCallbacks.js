...
(function(u, undefined) {
    ...
    u.jobHoursCmds = function (shellFrame) {
        ///<summary>工时管理相关命令</summary>
        ...
        cmdJobRecordView.add2TaskGroup(shellFrame, groupId, 1);
        ...
        cmdJobLogNew.add2TaskGroup(shellFrame, groupId, 2);

        var isPM = CC.userRole.isPM(vault);//项目总监组
        var isMajordomo = CC.userRole.isMajordomo(vault);//项目经理组
        if (isMajordomo || isPM) {
            var cmdBudgetHour = new MF.Command("预算工时", 'icons/项目成员忙闲度.ico');
            cmdBudgetHour.create(shellFrame);
            cmdBudgetHour.registerEvent(shellFrame, function (sf) {
                CC.budgetHours.budgetHourCmdFn(sf, isPM);
            });
            cmdBudgetHour.add2TaskGroup(shellFrame, groupId, 3);

            var cmdHoursReport = new MF.Command("报表统计", 'icons/项目报表.ico');
            cmdHoursReport.create(shellFrame);
            cmdHoursReport.registerEvent(shellFrame, function (sf) {
                //sf.CurrentPath = "项目报表";
                CC.hoursReport.hoursReportCmdFn(sf, isPM);
            });
            cmdHoursReport.add2TaskGroup(shellFrame, groupId, 4);
        }
    };

    u.infoNoticeCmds = function(shellFrame) {
        ///<summary>消息管理相关命令</summary>
        var vault = shellFrame.ShellUI.Vault;
        var groupName = "消息管理";
        var groupId = shellFrame.TaskPane.CreateGroup(groupName, -5);

        var cmdInfoNoticeView = new MF.Command("查看通知", 'icons/查看工作日志.ico');
        cmdInfoNoticeView.create(shellFrame);
        cmdInfoNoticeView.registerEvent(shellFrame, function (sf) {
            //sf.ShellUI.ShowMessage("未实现！");
            sf.CurrentPath = "消息通知";
        });
        cmdInfoNoticeView.add2TaskGroup(shellFrame, groupId, 1);

        var cmdInfoNoticeNew = new MF.Command("新建通知", 'icons/新建工作日志.ico');
        cmdInfoNoticeNew.create(shellFrame);
        cmdInfoNoticeNew.registerEvent(shellFrame, function (sf) {
            //sf.ShellUI.ShowMessage("未实现！");
            var typeId = MF.alias.objectType(vault, md.infoNotice.typeAlias);
            var objType = vault.ObjectTypeOperations.GetObjectType(typeId);
            vault.ObjectOperations.ShowBasicNewObjectWindow(0, objType);
        });
        cmdInfoNoticeNew.add2TaskGroup(shellFrame, groupId, 2);
    };
    ...
    u.createProjCmds = function (shellFrame) {
        ...
        infoCmd.registerEvent(shellFrame, function (sf) {
            ...
            var vault = shellFrame.ShellUI.Vault;
            var srcParties = [];
            var hasParty = MF.alias.objectType(vault, md.participant.typeAlias) !== -1;
            if (hasParty) {
        ...
        var cmdModelPlan = new MF.Command("模型组织配置", 'icons/模型策划.ico');
        cmdModelPlan.create(shellFrame);
        cmdModelPlan.registerEvent(shellFrame, function (sf) {
            sf.CurrentPath = "BIM模型管理\\模型策划";
        });
        cmdModelPlan.add2TaskGroup(shellFrame, projManagerId, 3);
        ...
        inviteCmd.registerEvent(shellFrame, function (sf) {
            ...
            //var srcParts = [];
            //if (MF.alias.objectType(vault, md.participant.typeAlias) !== -1) { //'ObjParticipant'
            ...
            //}
            //var inviteEmail = "";
            //var dashboardData = {Vault:vault, SrcParts: srcParts, SelectedPart: selectedPart, InviteEmail: inviteEmail, Cancelled: true };
            //shellFrame.ShowPopupDashboard("invitemember", true, dashboardData);

        });
        inviteCmd.add2TaskGroup(shellFrame, projManagerId, 4);
    }
    ...
    u.createTaskCmds = function (shellFrame) {
        ...
        taskCmdNew.registerEvent(shellFrame, function (sf) {
            ...
            MF.ui.createNewObjectShowWindow(sf, MFBuiltInObjectTypeAssignment,
                { type: MFBuiltInPropertyDefClass, value: MFBuiltInObjectClassGenericAssignment });
        });
    ...
    u.createDocumentCmds = function (shellFrame) {
        ...
        templateCmd.add2TaskGroup(shellFrame, docManagerId, 3);
    };
    ...
    u.createCmds = function (shellFrame, globalObject) {
        ...
        if (CC.userRole.isOutsourceMember(vault) === false) {
            ...
            */
            //工时管理
            var hasBudgetHour = MF.alias.objectType(vault, md.totalBudgetHours.typeAlias) !== -1;
            if (hasBudgetHour) {
                this.jobHoursCmds(shellFrame);
            }
        }
        ...
        var hasInfoNotice = MF.alias.objectType(vault, md.infoNotice.typeAlias) !== -1;
        if (hasInfoNotice) {
            this.infoNoticeCmds(shellFrame);
        }
    ...
    u.createContextMenuCmds = function (shellFrame, globalObject) {
        ...
        cmdOpenAtCloud.add2ContextMenu(shellFrame, MenuLocation_ContextMenu_Top, 2);
    ...
    u.setCmdSateAsSelectionChanged = function (shellFrame) {
        ...
        try {
            ...
            if (hasParty) {
                //设置“共享给”按钮显示状态
                var objVns = shellFrame.Listing.CurrentSelection.ObjectVersions;
                ...
                //选择"虚拟文件夹"时，设置"共享给"按钮状态，项目文档\工作文档\**方...下显示
                    ...
                    var currentPath = shellFrame.Listing.CurrentPath;
                    var flag = false;
                    if (currentPath) {
    ...
    u.addTabAsSelectionChanged = function(shellFrame) {
        ...
        try {
            ...
            if (reportType !== -1) {
                //当选中"项目报表"对象时，添加Tab
                var objVnsPros = shellFrame.Listing.CurrentSelection.ObjectVersionsAndProperties;
