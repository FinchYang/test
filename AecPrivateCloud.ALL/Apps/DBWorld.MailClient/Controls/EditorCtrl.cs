...
{
    public partial class EditorCtrl : UserControl, ISearchableBrowser
    {
        ...
        private List<Attachment> _attachList = new List<Attachment>();
        ...
        public delegate void NavigatingEventHandler(object sender, WebBrowserNavigatingEventArgs e);

        public event NavigatingEventHandler WbNavigating;

        protected virtual void OnWbNavigating(object sender, WebBrowserNavigatingEventArgs e)
        {
            NavigatingEventHandler handler = WbNavigating;
            ...
            foreach (Match match in matches)
            {
                ...
                if (File.Exists(src))
                {
                    ...
                    if (ext.Length > 0)
                    {
                        ...
                        res.ContentType.MediaType = GetMimeType('.' + ext);
            ...
            foreach (Match match in matches)
            {
                ...
                if (File.Exists(src))
                {
                    ...
                    if (ext.Length > 0)
                    {
                        ext = ext.Substring(1);
                        ext = GetMimeType(ext);
                        src = string.Format("'data:{0};base64,{1}'", ext, Convert.ToBase64String(File.ReadAllBytes(src)));
                        html = html.Replace(match.Groups[1].Value, src);
                    }
                }
            }
        ...
        private void Document_ContextMenuShowing(object sender, HtmlElementEventArgs e)
        {
            ...
            cSSToolStripMenuItem.Enabled = SelectionType != DBWorld.MailClient.SelectionType.None;
        ...
        /// <summary>
        /// Insert html at the header
        /// </summary>
        /// <param name="html">The div content</param>
        public void InsertBeginHtml(string html)
        {
            ...
            if (document != null)
            {
                ...
                if (divElem != null)
                {
                    ...
                    if (document.Body != null)
                        divElem = document.Body.InsertAdjacentElement(HtmlElementInsertionOrientation.AfterBegin,
                            divElem);
                }
            }
        }
        ...
        public FontSize FontSize
        {
            get
            {
                if (ReadyState != ReadyState.Complete)
                    return FontSize.NA;
                string temp = doc.queryCommandValue("FontSize").ToString();
                switch (temp)
                {
                    case "1":
                        return FontSize.One;
        ...
        {
            if (SelectionType != DBWorld.MailClient.SelectionType.Text) return false;
        ...
        public void SelectLink()
        {
            ...
            {
                case DBWorld.MailClient.SelectionType.Control:
                    {
                    ...
                    break;
                case DBWorld.MailClient.SelectionType.Text:
                    {
                        IHTMLTxtRange txtRange = (IHTMLTxtRange)doc.selection.createRange();
                        if (txtRange != null && !string.IsNullOrEmpty(txtRange.htmlText))
                        {
        ...
        private void attchButton_Click(object sender, EventArgs e)
        {
            //选择文件
            ...
            {
                if (this.IsSize(openFileDlg.FileName))
                { 
                    AddAttachment(openFileDlg.FileName);
                } 
            } 
        }
        ...
        /// </summary>
        public void AddAttachment(string pathFileName)
        {
            if(pathFileName == string.Empty)return;
            var extName = Path.GetExtension(pathFileName).ToLower();
            ...
            _attachList.Add(attachment);

            this.txtBoxAttach.Text = String.Empty;
            foreach (var item in _attachList)
            {
                this.txtBoxAttach.Text += item.Name;
                this.txtBoxAttach.Text += ';';
                this.txtBoxAttach.SelectionStart = this.txtBoxAttach.TextLength;
                this.txtBoxAttach.SelectionLength = 0;
            }
        }

        private int _deleteIndex = 0;
        private void txtBoxAttach_Click(object sender, EventArgs e)
        {
            var address = new EmailAddressUtil(txtBoxAttach.Text, ';', txtBoxAttach.SelectionStart);
            int nStart = 0, nLen = 0;
            address.GetSelectedItemSection(ref nStart, ref nLen);
            txtBoxAttach.SelectionStart = nStart;
            txtBoxAttach.SelectionLength = nLen;
            _deleteIndex = address.GetSelectedItemIndex();
        }

        private void txtBoxAttach_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == (char)Keys.Back)
            {
                ...
                //删除附件
                if (ctrl.SelectionLength == ctrl.TextLength)
                {
                    _attachList.Clear();
                }
                else
                {
                    ...
                    {
                        _attachList.RemoveAt(_deleteIndex);
                    }
                }
            }
            else
        ...
        private void txtBoxAttach_KeyDown(object sender, KeyEventArgs e)
        {
            var ctrl = sender as TextBox;
        ...
        private void txtBoxAttach_DoubleClick(object sender, EventArgs e)
        {
            var ctrl = sender as TextBox;
        ...
        /// <param name="e"></param>
        private void txtBoxAttach_DragEnter(object sender, DragEventArgs e)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void txtBoxAttach_DragDrop(object sender, DragEventArgs e)
        {
            var fileName = ((System.Array)e.Data.GetData(DataFormats.FileDrop)).GetValue(0).ToString();
            if (this.IsSize(fileName)) AddAttachment(fileName);
        }
    }
