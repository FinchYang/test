...
(function(u, undefined){
    var vault = {
        ...
        getWorkflowState: function(mfault, objVer) {
            var wf = mfault.ObjectPropertyOperations.GetWorkflowState(objVer, false);
            var flowId = wf.Workflow.Value.DisplayValue ? wf.Workflow.Value.GetLookupID() : -1;
            var stateId = wf.State.Value.DisplayValue ? wf.State.Value.GetLookupID() : -1;
            //修改人
            var operatedBy = mfault.ObjectPropertyOperations.GetProperty(objVer, MFBuiltInPropertyDefLastModifiedBy).Value.DisplayValue;
            //修改时间
            var operatedTime = mfault.ObjectPropertyOperations.GetProperty(objVer, MFBuiltInPropertyDefLastModified).Value.DisplayValue;
            return {
                flowId: flowId,
                stateId: stateId,
                comment: wf.VersionComment.Value.DisplayValue,
                versionId: objVer.Version,
