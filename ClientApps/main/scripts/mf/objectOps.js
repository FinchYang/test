...
(function(v) {
    var o = {
		...
		createObjPrefilled: function(vault, prefilledPropValues, typeId, sourceObjectFiles, forceCheckin){
			var pvs;
            ...
            oCreationInfo.SetObjectType(typeId, false);
			if(sourceObjectFiles) oCreationInfo.SetSourceFiles(sourceObjectFiles);
            var objResult = vault.ObjectOperations.ShowPrefilledNewObjectWindow(0, MFObjectWindowModeInsert,
                oCreationInfo, pvs, MFiles.CreateInstance("AccessControlList"));
            if(forceCheckin && objResult.Result === MFObjectWindowResultCodeOk){
                var isCheckOut = vault.ObjectOperations.IsObjectCheckedOut(objResult.ObjVer.ObjID, false);
                if(isCheckOut){
                    vault.ObjectOperations.CheckIn(objResult.ObjVer)
                } 
            }
		},
        ...
        SearchObjects: function (vault, typeId, searchConditions) {
                ...
                if (typeId === -1) {
                MFiles.ThrowError('非法的对象类型ID： '+typeId);
            }
        ...
        ///<summery>更新对象,更新失败返回null</summery>
        updateObject: function (vault, objectVersion, propValues) {
            ...
            try {
                var res = vault.ObjectOperations.CheckIn(checkOutVn.ObjVer);
                return res;
            } catch (e) {
