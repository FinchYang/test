...
(function (u, undefined) {
    var category = { 
        /*function:创建零时视图
         * parentId:父容器Id
         * createPath：创建路径 例：系统邮件//收件箱//...ui-layout-center
         * viewId：搜索出的视图Id*/
        create_dashboard: function (shellFrame, loc, parentId , createPath, vault, viewId) {
                try {
                  var listingElem = '<div id="listings" class="mf-panel-container" style="min-width: 10px;">' +
                              '<div id="listing_temp" class="mf-panel" style="padding: 0px;margin: 0px; border: 0px; overflow: hidden !important;">' +
                              '<div class="mf-listing-content heightcontrol" style=" overflow-x: auto !important; overflow-y: hidden !important;'+
                              ' min-width: 10px; ">' +
                              '</div>' +
                              '</div>' +
                              '</div>';
